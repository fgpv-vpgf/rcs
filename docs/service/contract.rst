...
GET ``/v2/docs/[lang]/[smallkey]{,[smallkey]}{/[sortarg]}``
-----------------------------------------------------------
...
- the service URL should not have any query string component
- ``metadata``, ``display_field``, ``service_name``, ``loading_mode``,
``max_allowable_offset``, ``tolerance`` are optional
- where ``service_url`` specifies Feature Layer endpoint, this may be an
  endpoint from a *Feature Service* or a *Map Service*; what is important is
  that the URL should reference the layer id within it
...
.. code-block:: javascript

    {
        ...
        "service_type": (str: Service type of the endpoint, either ogcWms or ogcWmts),
        ...
        "recursive": (bool: Indicats if children should be scraped and made available
...
Request Params: smallkey for an already registered v1 layer
...
Updates all v1 records in the database to v2 format.
